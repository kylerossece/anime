...
export default async function Page() {
  const trending = (await getAnime(
    query({ sortType: "TRENDING_DESC", page: 1, perPage: 50, type: "MANGA" })
  )) as PageResponse | null;
  const popular = (await getAnime(
    query({ sortType: "POPULARITY_DESC", page: 1, perPage: 50, type: "MANGA" })
  )) as PageResponse | null;
  const top = (await getAnime(
    query({ sortType: "SCORE_DESC", page: 1, perPage: 50, type: "MANGA" })
  )) as PageResponse | null;
